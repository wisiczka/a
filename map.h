...
/*
  Clears the map and mob list so we can start working with it.
*/
void init_map()
{
  int y;
...
void populate(int t, int l, int b, int r)
{
  ...
  {
    mob[next_mob].y = t + rand() % MAX(1, (b - t));
    mob[next_mob].x = l + rand() % MAX(1, (r - l));

    if ((blocking(mob[next_mob].y, mob[next_mob].x) & CANT_MOVE) == 0)
    {
      mob[next_mob].type = type;
      next_mob++;
    }
